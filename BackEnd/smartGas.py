...
def read_sensor(sensor_id, sensor_name, register_address, quantity, ser, button_id, calibration_in_progress, conc_cal):
    ...
    try:
        ...
        # 84 Adres Sorgusunda Manueldeki Formülde Bulunan Span_old Değeri ve write_to_sensor fonksiyonuna yönlendirme
        else:
...
def write_to_sensor(sensor_id, register_address, ser, value_to_write):
    ...
    try:
        ...
        response = ser.read(8)  # 8 byte uzunluğunda yanıt bekleniyor
...
def read_and_calibrate_sensor(sensor_id, sensor_name, button_id, calibration_in_progress, conc_cal):
    global ser
    try:
        ...
        if button_id in ['CObuttonspan', 'CO2buttonspan', 'CH4buttonspan']:
            if calibration_in_progress:        
                if button_id == 'CObuttonspan':
                    read_and_calibrate_sensor(43, "%CO", button_id, calibration_in_progress, conc_cal)
                elif button_id == 'CO2buttonspan':
                    read_and_calibrate_sensor(2, "%CO2", button_id, calibration_in_progress, conc_cal)
                elif button_id == 'CH4buttonspan':
                    read_and_calibrate_sensor(79, "%CH4", button_id, calibration_in_progress, conc_cal)
                else:
